...
// SQL Workload RCA - Enterprise Edition
// ===============================================

let currentAnalysisData = null;
let fixPanelsState = {}; // Track which Fix panels are expanded
...
async function initializeResultsPage() {
  ...
  try {
    // Load existing results (time window is auto-detected from AWR)
    await loadExistingResults();
  } catch (error) {
    console.error("Error in initializeResultsPage:", error);
    showNoDataMessage();
  }
}
...
async function loadExistingResults() {
  try {
    ...
    if (!response.ok) {
      console.log("API response not OK, showing no data message");
      showNoDataMessage();
      return;
    }
    ...
    if (result.has_data) {
      ...
      if (result.has_parsed_csv) {
        displayParsingResults(result);
      }

      // Show high load detection and set time window
      if (result.high_load_periods && result.high_load_periods.length > 0) {
        displayHighLoadDetection(result.high_load_periods);
      } else {
        // No high load periods - still show time window section with default message
        ...
        if (displayEl) {
          displayEl.textContent = "Not detected - no high load periods found";
        }
      }

      // Show time window section if it exists (removed from results.html, kept in newresults.html)
      const timeWindowSectionEl = document.getElementById("timeWindowSection");
      if (timeWindowSectionEl) {
        timeWindowSectionEl.style.display = "block";
      }

      // If we have previous RCA results, display them
      if (result.has_rca_result && result.analysis_results) {
        displayRCAResults(result.analysis_results);
      }
    } else {
      console.log("No data in result, showing no data message");
      showNoDataMessage();
    }
  } catch (error) {
...
function showNoDataMessage() {
  console.log("showNoDataMessage called");
  const main = document.querySelector(".results-main");
  if (!main) {
    console.error("Could not find .results-main element");
    return;
  }
  main.innerHTML = `
        <section class="enterprise-card">
            <div class="card-body" style="text-align: center; padding: 4rem;">
                ...
                <h2 style="color: #e2e8f0; margin-bottom: 1rem;">No Analysis Data Available</h2>
                <p style="color: #94a3b8; margin-bottom: 2rem;">Please upload AWR/ASH reports from the Dashboard to begin analysis.</p>
...
function displayParsingResults(result) {
  ...
  // Priority 1: Direct csv_file_list from API
  if (result.csv_file_list && Array.isArray(result.csv_file_list)) {
    csvFiles = result.csv_file_list;
    csvCount = csvFiles.length;
  }
  // Priority 2: csv_validation object
  else if (result.csv_validation?.csv_file_list) {
    csvFiles = result.csv_validation.csv_file_list;
    csvCount = csvFiles.length;
  }
  // Priority 3: parsing_results CSV entries
  else if (result.parsing_results && Array.isArray(result.parsing_results)) {
    csvFiles = result.parsing_results
      .filter((f) => f.file && f.file.endsWith(".csv"))
      .map((f) => f.file);
    csvCount = csvFiles.length;
  }

  // Fallback to csv_count field
  if (csvCount === 0) {
    csvCount = result.csv_count || result.csv_validation?.total_csv_files || 0;
  }

  // Count processed reports (non-CSV files)
  const processedReports =
    result.parsing_results?.filter(
      (p) => p.type !== "CSV" && !p.file?.endsWith(".csv"),
    ).length || 0;
  ...
  let html = `
        <div class="parsing-grid">
            <div class="parsing-stat">
                ...
                <span class="parsing-stat-label">CSV Files in System</span>
            </div>
            <div class="parsing-stat">
                <span class="parsing-stat-value">${processedReports || result.parsing_results?.length || 0}</span>
                <span class="parsing-stat-label">Files Processed</span>
            </div>
        </div>
    `;

  // Show CSV file list if available
  if (csvFiles.length > 0) {
    ...
    html += `</div>`;
  } else if (result.parsing_results && result.parsing_results.length > 0) {
    // Fallback to showing parsing results
    html += `<div class="parsing-files-list">`;
    result.parsing_results.forEach((file) => {
      ...
      html += `
                <div class="parsing-file-item ${statusClass}">
                    <span class="file-status-icon">${statusIcon}</span>
                    <span class="file-name">${file.file}</span>
                    <span class="file-type">${file.type}</span>
                    ${file.csv_files ? `<span class="file-csv-count">${file.csv_files} CSVs</span>` : ""}
                </div>
            `;
    });
    html += `</div>`;
...
function displayHighLoadDetection(highLoadPeriods) {
  ...
  // Filter AWR-based high load only
...
function _legacy_displayDBAExpertAnalysis(dbaAnalysis) {
  ...
  if (findings.length > 0) {
    ...
    findings.forEach((finding, index) => {
      ...
      // Debug logging
      console.log(
        `[LoadReduction] Finding ${index}: sql_id=${sqlId}, hasLRA=${hasLRA}, lra=`,
        lra,
      );
      ...
      html += `
                <div class="sql-finding-card" id="finding-${index}">
                    ...
                    <!-- ⚡ FIX RECOMMENDATIONS - UI-READY EXECUTABLE ACTIONS -->
                    ${generateFixRecommendationsSection(sqlId, finding)}
                    ...
                    <!-- Analysis Details Button - Collapsed by Default -->
                    <div class="fix-button-container">
                        <button class="btn-fix" onclick="toggleFixPanel('${fixPanelId}', ${index})">
    ...
    html += `
            <div class="no-findings-message">
                ...
                <p>Database workload appears normal. No high-risk queries detected.</p>
            </div>
        `;
  }
...
function toggleFixPanel(panelId) {
  ...
  if (panel.classList.contains("collapsed")) {
    ...
    btn.querySelector("span").textContent = "Hide Detailed Analysis";
    ...
    btn.querySelector("span").textContent = "Show Detailed Analysis";
  }
}

// ===============================================
// LOAD REDUCTION PANEL TOGGLE
// ===============================================
function toggleLoadReductionPanel(panelId) {
  const panel = document.getElementById(panelId);
  const container = panel.parentElement;
  const btn = container.querySelector(".btn-load-reduction");
...
WHERE signature = (SELECT exact_matching_signature FROM v$sql WHERE sql_id = '${sqlId}' AND ROWNUM = 1);</code></pre>
  ...
  // 🔥 NEW: Generate Load Reduction Actions section
  html += generateLoadReductionSection(sqlId, finding);
...
// ===============================================
// 🔥 LOAD REDUCTION ASSISTANT - NEW SECTION
// ===============================================
// This system is no longer just an analysis tool.
// It is a Load Reduction Assistant.
function generateLoadReductionSection(_sqlId, finding) {
  const loadReductionActions = finding?.load_reduction_actions;

  if (
    !loadReductionActions ||
    !loadReductionActions.actions ||
    loadReductionActions.actions.length === 0
  ) {
    return ""; // No load reduction actions available
  }

  const detectedCauses = loadReductionActions.detected_root_causes || [];
  const actions = loadReductionActions.actions || [];

  let html = `
        <div class="fix-section load-reduction-section">
            <div class="fix-section-header load-reduction-header">
                <span class="fix-icon">⚡</span>
                <h4>DBA Actions to Reduce Load</h4>
                <span class="load-reduction-badge">${actions.length} Action${actions.length > 1 ? "s" : ""}</span>
            </div>
            <div class="fix-section-content">
                <div class="load-reduction-intro">
                    <p class="load-reduction-purpose">
                        These queries help <strong>REDUCE database load</strong> by enabling you to fix 
                        IO, CPU, PX, plan, and indexing issues at the SQL level.
                    </p>
                </div>
    `;

  // Generate each action section
  for (const action of actions) {
    const categoryClass = getCategoryClass(action.category);
    const categoryIcon = getCategoryIcon(action.category);

    html += `
            <div class="load-reduction-action ${categoryClass}">
                <div class="action-header">
                    <span class="action-icon">${categoryIcon}</span>
                    <h5 class="action-title">${action.title}</h5>
                    <span class="priority-badge priority-${action.priority}">Priority ${action.priority}</span>
                </div>
                
                <div class="action-why-helps">
                    <strong>Why This Helps:</strong>
                    <p>${action.why_this_helps}</p>
                </div>
                
                <div class="action-queries">
                    <div class="queries-header">
                        <span class="queries-icon">📋</span>
                        <strong>DBA-Executable Queries:</strong>
                    </div>
                    ${action.sql_queries
                      .map(
                        (query) => `
                        <div class="query-block">
                            <pre class="sql-code load-reduction-sql"><code>${escapeHtml(query)}</code></pre>
                            <button class="copy-sql-btn" onclick="copySqlToClipboard(this)" title="Copy to clipboard">
                                📋 Copy
                            </button>
                        </div>
                    `,
...
// Helper function to format root cause category names
function formatRootCause(cause) {
  const causeMap = {
    IO_DOMINANT: "🔴 IO Dominant",
    PX_INEFFECTIVE: "⚡ PX Ineffective",
    BAD_EXECUTION_PLAN: "📌 Bad Plan",
    HIGH_CPU: "🔥 High CPU",
...
function getCategoryClass(category) {
  const classMap = {
    IO_DOMINANT: "category-io",
    PX_INEFFECTIVE: "category-px",
    BAD_EXECUTION_PLAN: "category-plan",
    HIGH_CPU: "category-cpu",
    MISSING_INDEX: "category-index",
...
// DBA-Executable Actions in UI-Ready Format
// Matches DBA Action Plan style EXACTLY
// ===============================================

function generateFixRecommendationsSection(_sqlId, finding) {
  const fixRecs = finding?.fix_recommendations;

  // If no fix_recommendations, return empty
  ...
  const totalSections = fixRecs.total_sections || sections.length;

  // Use EXACT same styling as "DBA ACTIONS TO REDUCE DATABASE LOAD"
  let html = `
...
function getCategoryColor(category) {
  const colorMap = {
    IO_REDUCTION: "#ef4444", // Red
    SQL_ACCESS_ADVISOR: "#f59e0b", // Orange
    PARALLEL_EXECUTION: "#3b82f6", // Blue
    PLAN_STABILITY: "#8b5cf6", // Purple
    CPU_REDUCTION: "#f97316", // Orange-Red
  };
